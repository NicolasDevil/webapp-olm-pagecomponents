<template>
  <div class="el-table"
    ...
    <div class="el-table__header-wrapper" ref="headerWrapper" v-if="showHeader"  role="rowgroup">
      <table-header
        :store="store"
        :layout="layout"
        :border="border"
    ...
    <div
      ...
      <div :style="{ width: bodyWidth }" class="el-table__empty-block" v-if="!data || data.length === 0">
        ...
        <div v-if="emptyIcon" class="el-table__empty-icon"><i class="icon-ng-phiz-error"/></div>
        <span><slot name="empty">{{ emptyText || t('el.table.emptyText') }}</slot></span>
        </div>
      </div>
    </div>
