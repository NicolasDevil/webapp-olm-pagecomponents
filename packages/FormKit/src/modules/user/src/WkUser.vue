<template>
  <div class="xh-user">
    <div v-if="headerShow" class="xh-user__hd">员工</div>
    ...
    <div class="xh-user__ft">
      <span class="select-info">
        已选择
        <span class="select-info--num">{{ dataValue ? dataValue.length : 0 }}</span>项
      </span>
      <div class="btn">
        <el-button type="text" @click="clearAll">清空</el-button>
        <el-button type="text" @click="submitHandle">确定</el-button>
      </div>
    </div>
  </div>
</template>
  ...
  watch: {
    value() {
      if (this.options && !this.radio) {
        ...
        if (
          this.value.length == optionsLength &&
          (this.value.length > 0 || optionsLength > 0)
        ) {
          this.checkAll = true;
        } else {
          this.checkAll = false;
