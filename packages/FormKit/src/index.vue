<template>
  <div :class="[{ 'form-label-auto': labelWidth === 0 }]" class="FormKit">
    <el-form
      ...
      <el-row v-bind="row" :style="{ 'row-gap': `${rowGap}px` }" class="form-kit-row">
        <el-col
...
export default {
  ...
  async created() { this.Initialisation() },
  methods: {
    Initialisation() {
      ...
      return new (class {
        ...
        init(conf, FUNC = ['common']) {
          try {
            for (const iterator of conf) { FUNC.map(name => (this[name] ? this[name](iterator) : _.noop())) }
            THIS.Notes.__PENGDING = this.pending
            THIS.executeStack()
          } catch (error) {
            console.log(`[_initComponent method]: ${error}`)
          }
        }
        common(config, __opt = ['select', 'radio', 'cascader', 'address', 'checkbox']) {
          const { type } = config
      ...
      if (_.isEmpty(config.visible)) {
        ...
        if (_.isObject(config.visible) || _.isArray(config.visible)) {
          ...
          if (_.isArray(config.visible)) {
            ...
            const isCheck = ARR.some(it => { return this.FormData[it.key] === it.value })
            if (isCheck) return config
          }
        } else {
          console.warn('visible field has been set, but it is not an array or object!')
          return config
        }
      }
    },
    ...
    verificationProps(rows = {}) {
      ...
      if (type === 'address') {
        const _props = Object.assign({ value: 'code', label: 'name', children: 'children' }, props.props)
    ...
    async getDistricts() {
      const _OPT = async() => {
        try {
          const districts = await import(`@/components/VDistpicker/districts.js`)
...
.FormKit {
  ...
  /deep/ .el-input-number { width: auto }
